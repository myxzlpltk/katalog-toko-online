...
@section('content')
	<div class="card shadow mb-3">
		...
		<div class="card-body">
			<form action="{{ route('admin.dosens.update', $dosen) }}" method="post" enctype="multipart/form-data">
				@csrf
				@method('PATCH')
				...
				<div class="form-group">
					...
					<input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $dosen->user->name) }}" required>
					@error('name')
					<div class="invalid-feedback">{{ $message }}</div>
					@enderror
				</div>
